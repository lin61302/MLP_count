...
# Convert Series to lists before passing to the function
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    for date in df.index:
        ...
        proc = multiprocessing.Process(target=update_info(docs, event_types, keywords, colname), args=(docs, event_types, keywords))
        proc.start()
        proc.join() 
        ...
        for et in events:
            ...
            # Apply the check_georgia filter if the country code is 'GEO'
            if country_code == 'GEO':
                # Function to handle the GEO filtering for both cases
                def apply_loc_geo_filter(index, doc):
                ...
                count = sum(
                    ...
                    and apply_loc_geo_filter(index, doc)
                )
...
def count_domain_int(uri, domain, country_name, country_code):
    ...
    for date in df.index:
        ...
        #             event_types.append(doc['RAI']['event_type'])
        #             if check_rai(doc['maintext_translated']) or check_rai(doc['title_translated']):
        #                 text = doc['title_translated'] + '. ' + doc['maintext_translated']
