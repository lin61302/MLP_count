...
def update_info(docs, event_types, keywords, colname):
    ...
    for nn, _doc in enumerate(docs):
        ...
        db_local[colname].update_one(
            {'_id': _doc['_id']},
            {'$set': {
                'event_type_RAI_new': event_types[nn],
                'RAI_new_keywords': keywords[nn]
            }}
        )
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Counts_RAI_New/{country_name}/{today.year}_{today.month}_{today.day}/Combined/'
