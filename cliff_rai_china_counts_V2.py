...
def update_info(docs, event_types, colname):
    ...
    for nn, _doc in enumerate(docs):
        ...
        db_local[colname].update_one(
            {'_id': _doc['_id']},
            {'$set': {'event_type_RAI_China': event_types[nn]}}
        )
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    for date in df.index:
        ...
        for et in events:
            if et == '-999':
                ...
                count = sum(1 for i, d in enumerate(docs) if d['RAI'].get('event_type') == et and event_types[i] == et)
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Counts_RAI/{country_name}/{today.year}_{today.month}_{today.day}/China/'
...
if __name__ == "__main__":
    ...
    countries_needed = ['CMR','TUN','LKA','UGA','NPL','AGO']
