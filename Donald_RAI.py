...
def count_domain_loc(uri, domain, country_name, country_code):
    db = MongoClient(uri).ml4p
    df = pd.DataFrame()
    df['date'] = pd.date_range('2021-8-1', '2022-9-1', freq='M')
    df.index = df['date']
    ...
    for date in df.index:
        ...
        for index, doc in enumerate(docs):
            ...
            try:
                if check_rai(doc['maintext_translated'][:2000]) or check_rai(doc['title_translated']) or check_title(doc['title_translated']) or check_title(doc['maintext_translated']):
        ...
        for et in events:
            ...
            for doc in docs:
                try:
                    ...
                    pass
            sub_docs = [doc for doc in sub_docs if check_rai(doc['maintext_translated'][:2000]) or check_rai(doc['title_translated']) or check_title(doc['title_translated']) or check_title(doc['maintext_translated'])]
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Donald_RAI/{country_name}/{today.year}_{today.month}_{today.day}/'
    
    if not os.path.exists(path):
        Path(path).mkdir(parents=True, exist_ok=True)
    df_articles = pd.DataFrame(docs_articles)
    df_articles.to_csv(path + f'{domain}.csv')
...
#             try:
#                 if check_rai(docs[index]['maintext_translated']) or check_rai(docs[index]['title_translated'])  or check_title(doc['title_translated']):
#                     event_types.append(doc['RAI']['event_type'])
...
# Albania, Azerbaijan, Armenia, Belarus, Georgia, Hungary, Kazakhstan, Kosovo, Kyrgystan, Moldova, Serbia, Turkey, Ukraine
    # 1110
