...
events = list(db.models.find_one({'model_name': 'RAI_new'}).get('event_type_nums').keys())
...
def update_info(docs, event_types, colname):
    ...
    for nn, _doc in enumerate(docs):
        ...
        db_local[actual_col].update_one(
            {'_id': _doc['_id']},
            {'$set': {
                'event_type_RAI_new_Russia': event_types[nn]
            }}
        )
...
def count_domain_loc(uri, domain, country_name, country_code):
        ...
        for et in events:
            if et == '-999':
                ...
                count_val = sum(1 for idx, d in enumerate(docs)
                                if d['RAI_new'].get('result') == et and event_types[idx] == et)
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Counts_RAI_New/{country_name}/{today.year}_{today.month}_{today.day}/Russia/'
...
if __name__ == "__main__":
    ...
    countries_needed = [
        'ALB', 'BEN', 'COL', 'ECU', 'ETH', 'GEO', 'KEN', 'PRY', 'MLI', 'MAR', 'NGA', 'SRB', 'SEN', 'TZA', 'UGA', 'UKR', 
            'ZWE', 'MRT', 'ZMB', 'XKX', 'NER', 'JAM', 'HND', 'PHL', 'GHA', 'RWA', 'GTM', 'BLR', 'KHM', 'COD', 'TUR', 'BGD', 
            'SLV', 'ZAF', 'TUN', 'IDN', 'NIC', 'AGO', 'ARM', 'LKA', 'MYS', 'CMR', 'HUN', 'MWI', 'UZB', 'IND', 'MOZ', 'AZE', 
            'KGZ', 'MDA', 'KAZ', 'PER', 'DZA', 'MKD', 'SSD', 'LBR', 'PAK', 'NPL', 'NAM', 'BFA', 'DOM', 'TLS', 'SLB', 'CRI', 
            'PAN', 'MEX'
                                 ]
    ...
    for ctup in countries:
        ...
        commit_message = f"RAI New Russia count ({countries_added}) update"
