...
protest_re =  re.compile(r'((?<![a-z])corrupt|(?<![a-z])corruption|(?<![a-z])embezzlement|(?<![a-z])bribe|(?<![a-z])bribes|(?<![a-z])bribery|(?<![a-z])bribing|influence peddling|nepotism|nepotistic|illicit financial flows)',flags=re.IGNORECASE)
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Counts_protest_with_corruption/{country_name}/{today.year}_{today.month}_{today.day}/'
