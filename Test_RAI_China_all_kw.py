...
__chinapath__ = '/home/ml4p/Dropbox/Dropbox/ML for Peace/Acronyms/China_RAI_keywords_V1.xlsx'
...
for i, doc in enumerate(ch):
    if bool(int(ch_ind[i])):
        ...
        ch[i] = r"(?<![a-zA-Z])" + ch[i][2:-2].rstrip().lstrip() + r"(?![a-zA-Z])"
...
def update_info(docs, event_types, keywords, colname):
    ...
    for nn, _doc in enumerate(docs):
        ...
        db[colname].update_one(
            ...
            {
                '$set':{
                     'Test_RAI_China_all_kw':event_types[nn],
                    'Test_RAI_keywords_China_all_kw': keywords[nn]
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Test_Counts_RAI/original_kw/{country_name}/{today.year}_{today.month}_{today.day}/China/'
