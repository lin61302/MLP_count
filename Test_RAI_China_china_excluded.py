...
__chinapath__ = '/home/ml4p/Dropbox/Dropbox/ML for Peace/Acronyms/China_RAI_keywords_V3_china_excluded.xlsx'
...
def update_info(docs, event_types, keywords, colname):
    ...
    for nn, _doc in enumerate(docs):
        ...
        db[colname].update_one(
            ...
            {
                '$set':{
                    'Test_RAI_China_china_chinese_first_two':event_types[nn],
                    'Test_RAI_keywords_China_china_chinese_first_two': keywords[nn]
...
def count_domain_loc(uri, domain, country_name, country_code):
    ...
    for date in df.index:
        ...
        for index, doc in enumerate(docs):
            ...
            try:
                if check_rai(doc['maintext_translated'][:2000]) or check_rai(doc['title_translated']) or check_title(doc['title_translated']) or check_title(doc['maintext_translated'][:1200]):
                    ...
                    if check_title(doc['maintext_translated'][:600]):
                        content3 = title_re.findall(doc['maintext_translated'][:1200])
                        content3 = list(set(content3))
                        content3 = [ele for ele in content3 if len(str(ele).strip())>0]
                        keywords[index].extend(content3)
    ...
    path = f'/home/ml4p/Dropbox/Dropbox/ML for Peace/Test_Counts_RAI/china_first_two_sents/{country_name}/{today.year}_{today.month}_{today.day}/China/'
